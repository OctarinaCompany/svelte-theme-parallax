...
- Tabs hang off the bottom rule: list `-mb-6 flex`, links `relative … py-6` carrying
  ...
  the header rule, replacing that segment. (Marks that RUN vertically, like the sidebar's active
  bar, are 5px: a stroke lying down reads heavier than the same one standing up.) A pseudo-element
  rather than a border: a border can only be curved on the anchored edge, and 6px of it would eat
  the link's height.
