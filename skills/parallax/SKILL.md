...
| The page's scrollbar themed, or handed back to the OS | `page-scrollbar` from `parallax-appearance`: on by default, writes `data-scrollbar="themed"` on `<html>`, and `setPageScrollbar(false)` takes it off. It dresses the canvas's bar only — `scrollbar-gutter: stable` is separate, unconditional and part of `parallax-shell`, because reserving the width is a fix rather than a look |
