...
| A control inside an inverted or vibrant sidebar / header looks like it belongs to the page | The wear rule — see `references/theming.md`. The kit projects eleven page tokens onto the chrome family for you, but `@custom-variant dark` lives in YOUR stylesheet (shadcn's `init` writes it), so a registry item cannot teach it that a chrome surface is a dark ground. Copy the variant from that reference, keep every branch at specificity (0,1,0), and remember `--input` is a hairline inside chrome, never a fill |
