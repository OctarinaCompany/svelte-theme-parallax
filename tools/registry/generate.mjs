...
const APPEARANCE = {
	...
	docs: [
		...
		"    // The page scrollbar: off unless the reader asked for it.",
		'    if (localStorage.getItem("page-scrollbar") === "true") root.setAttribute("data-scrollbar", "themed");',
		...
		"",
		"It is OFF by default, like the backdrop: a scrollbar is a platform control before it is a design, so the kit offers to dress it rather than assuming the offer is accepted, and installing this item changes nothing until someone asks. `setPageScrollbar(true)` turns it on, and the switch on the gallery's Settings page is the worked example.",
		"",
...
const SHELL_CSS_SELECTORS = [
	...
	"@media print",
	// The bar is the canvas's footer on a phone: the geometry, the turned-over fade and hairline,
	// the reserve moved to the foot. One block, so the behaviour ships whole or not at all.
	"@media (width < 40rem)",
...
const SHELL = {
	...
	docs: [
		...
		"- Two touch-screen decisions ride on the same rules. The shell's root carries `touch-action: pan-x pan-y`, so a touch screen cannot pinch or double-tap zoom the shell — this is the declaration iOS Safari actually honours (it has ignored `user-scalable=no` for pinch since iOS 10), and it reaches a touch laptop too; pair it with the no-zoom viewport meta (`maximum-scale=1.0, user-scalable=no`), which no registry item can write for you and which still stops iOS zooming into a focused field. And the canvas carries `overflow-wrap: break-word`, inherited by every paragraph, so a word longer than its line — an identifier, a URL — breaks inside the word instead of overflowing the canvas and panning the whole page, sticky header included, sideways on a phone; tables and code blocks keep their own scrollers, and `break-word` leaves table min-content alone so cells never snap a token in half. Both are scoped to a document that holds the shell and are `:where()`-zero, so a sign-in page outside the shell keeps its zoom and either is taken back in plain CSS.",
		...
		"- Below 640px the bar is the canvas's FOOTER, not its header. One `@media (width < 40rem)` block from this item, scoped to `Sidebar.Inset`, moves `PageHeader` to the end of the canvas's column and holds it at the bottom edge (`order: 9999; top: auto; bottom: 0; margin-top: auto`), turns the floating inset, its fade and the flat hairline over with it, and moves the scroll reserve: `scroll-padding-bottom` replaces `-top` on the canvas, and the bar's cancellation becomes `scroll-margin-bottom`. Tablets keep the top bar — the line is Tailwind's `sm`, the one the bar's own layout already treats as the phone case. DOM and tab order do not move; only the paint does. Two consequences for your code: sticky chrome you parked at the reserve's depth needs `max-sm:top-0`, since on a phone the reserve is at the foot and nothing sits above it; and a header rendered outside `Sidebar.Inset` keeps its top. The auto-hide slide reads its direction from `--page-header-slide` (`100%` there), so a hand-rolled header wanting the same footer sets the variable rather than restating `translate`.",
