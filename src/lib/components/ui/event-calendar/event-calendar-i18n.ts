/**
 * Default UI texts, date-format strings, and formatter functions for the event calendar, fully
 * overridable per key. Ported 1:1 from the original source.
 */

import { format, isSameMonth, isSameYear, subMilliseconds, type Locale } from "date-fns";

import type { CalendarView, EventCalendarDateRange } from "./event-calendar-types.js";

export interface EventCalendarI18nConfig {
	labels: {
		today: string;
		previous: string;
		next: string;
	...
	formats: {
		...
		/** Agenda day-group header, leading weekday span (also opens the group aria-label). */
		agendaWeekday: string;
		/** Agenda day-group header, trailing full-date span. */
...
/**
 * Default formatting functions BOUND to a config's labels/formats, so that `formats` overrides
 * flow into the default renderers (a consumer overriding formats.monthTitle without replacing
 * formatTitle still sees it applied).
 */
function makeDefaultFunctions(
	...
	return {
		formatTitle: (view, { date, activeRange, locale }) => {
			...
			// subMilliseconds keeps the zoned date type (a plain new Date(ms) would flip the label
			// to the machine zone near midnight)
		...
		formatEventTime: (start, end, allDay, opts) => {
			...
			// Multi-day timed events carry the date on both sides. Compare calendar days off the
			// last rendered instant (end is exclusive, so a 14:00 to midnight event still ends on
			// the start day). Elapsed ms would miss an exactly-24h event and a DST day that only
			// runs 23 hours.
...
export const DEFAULT_EVENT_CALENDAR_I18N: EventCalendarI18nConfig = {
...
/**
 * One level deeper than `Partial`, because the merge below is per nested section: overriding a
 * single label must not force a consumer to restate the other 22. Unknown keys are still rejected
 * by the excess property check.
 */
export type EventCalendarI18nOverrides = {
	[K in keyof EventCalendarI18nConfig]?: Partial<EventCalendarI18nConfig[K]>;
};

/**
 * Shallow merge per nested object: a partial override replaces individual keys, never whole
 * sections. Default functions are re-bound to the MERGED labels/formats; explicit `functions`
 * overrides still win.
 */
export function mergeEventCalendarI18n(
