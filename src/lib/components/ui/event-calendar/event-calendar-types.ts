/**
 * Public TypeScript contract for the event calendar: events, occurrences, segments, state, and
 * callbacks. Ported from the original source with the React event types swapped for
 * DOM ones (`MouseEvent` instead of `React.MouseEvent`).
 */

export type EventCalendarEventId = string;

export const CALENDAR_VIEWS = ["month", "week", "day", "days", "agenda", "resource"] as const;

export type CalendarView = (typeof CALENDAR_VIEWS)[number];

/**
 * A bookable resource (room, person, equipment). Nesting via children renders with children; the
 * resource day view shows leaves as booking columns.
 */
export interface EventCalendarResource {
	id: string;
	title: string;
	/** Reserved for row/column accents; the built-in resource view does not render it yet. */
	color?: string;
...
export interface CalendarEvent<TData = unknown> {
	...
	/**
	 * Start and end must be midnights in the calendar's display time zone: segmentation walks the
	 * raw instants, so a row stored at another zone's midnight paints on the wrong days.
	 */
	allDay?: boolean;
...
export interface EventCalendarDragState<TData = unknown> {
	...
	/**
	 * True when the proposal was resolved on the day-granular surface (month cells / the all-day
	 * lane) rather than minute columns.
	 */
 ...
 * selection lives in EventCalendarSelection.slot.
 */
...
/**
 * User-adjustable display toggles. Every field is optional; undefined defers to the matching root
 * view-config prop.
 */
...
export interface EventCalendarStateSnapshot<TData = unknown> {
	view: CalendarView;
	/** Anchor date. */
	date: Date;
	/** For the "days" view. */
...
/**
 * Off-day marking (non-working days). `true` uses the defaults: weekends with a muted background.
 * Custom weekday sets, explicit dates, a predicate, and a custom class are all supported; marked
 * cells carry `data-off` for CSS-selector customization.
 */
...
/**
 * External-data contract: map a remote API's window query onto CalendarEvent rows. OAuth, tokens,
 * and sync loops are application backend territory.
 */
