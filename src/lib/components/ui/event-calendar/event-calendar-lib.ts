/**
 * Pure, framework-free calendar math: view ranges, zoned day keys, multi-day segmentation,
 * overlap packing, lane packing, and the event index. Pure functions throughout
...
import { expandRecurrence } from "./event-calendar-recurrence.js";
import type {
	CalendarEvent,
	CalendarView,
...
export function zonedStartOfDay(date: Date, timeZone: string): TZDate {
...
export function getDayTotalMinutes(dayStart: Date, timeZone: string): number {
...
export interface ViewDateRanges {
...
export function stepDate(
	...
	if (view === "month") {
		...
		// addMonths clamps the day down into a shorter month and never restores it, so
		// next-then-prev from the 31st would leave the anchor on the 28th. Sticking a month end to
		// the target month's end keeps stepping invertible, which matters because the anchor is
		// what day and week view open on after a month navigation.
...
export function rangesIntersect(a: EventCalendarDateRange, b: EventCalendarDateRange): boolean {
...
/**
 * The one canonical multi-day segmentation. Splits an occurrence into per-day segments clamped to
 * the range. Rules: exclusive end - an event ending exactly at zoned midnight emits NO segment
 * for that day; zero-duration events emit one min-height segment; allDay occurrences walk the
 * same absolute instants as timed ones and only drop startMin/endMin, so their bounds have to
 * already BE display-zone midnights (see CalendarEvent.allDay) or the bar paints on the wrong
 * days.
 */
export function segmentOccurrence<TData>(
...
export function isBarOccurrence(occurrence: EventCalendarOccurrence, timeZone?: string): boolean {
	return occurrence.allDay || spansMultipleDays(occurrence, timeZone);
}

export function spansMultipleDays(occ: { start: Date; end: Date }, timeZone?: string): boolean {
	// An event ending exactly at the next midnight is still single-day (exclusive end), so compare
	// against a strictly-later instant. The yardstick is the length of the day the event starts
	// on, never a flat 24h: a fall-back day is 25h long, and a 00:00-to-00:00 shift on it is still
	// one calendar day that belongs in the hour track, not in the all-day row. Without a display
	// zone the dates answer in their own frame (TZDate) or in the host zone.
...
/**
 * Google-style overlap packing for one day's timed segments. Mutates
 * column/columnCount/columnSpan on the segments, in place. z resolution happens at render:
 * event.zIndex verbatim, else 10 + column.
 */
export function packTimedSegments<TData>(segments: EventCalendarSegment<TData>[]): void {
...
/**
 * Build the laned month-row bars for one week: consecutive-day segments of the same occurrence
 * merge into ONE bar (colStart -> colSpan) stacked into lanes. Returns NEW segment objects - the
 * shared per-day segments (also rendered by the all-day rows and day cells) must stay pristine:
 * mutating their isEnd/continues flags gave the first-day chip a whole-bar shape and a bogus end
 * resize handle in the week all-day row.
 */
export function packWeekRowLanes<TData>(
...
export interface EventCalendarDayBucket<TData = unknown> {
	allDay: EventCalendarSegment<TData>[];
	timed: EventCalendarSegment<TData>[];
}

export interface EventCalendarWeekRow<TData = unknown> {
	rowIndex: number;
	rowStart: Date;
...
export interface EventCalendarIndex<TData = unknown> {
...
export interface BuildIndexOptions<TData> {
	timeZone: string;
	weekStartsOn: WeekStartsOn;
...
export function defaultEventOrder(a: EventCalendarOccurrence, b: EventCalendarOccurrence): number {
	return (
...
export function buildEventIndex<TData>(
	...
	// RECURRENCE-ID override replacement: an event carrying recurringEventId + originalStart is an
	// edited single occurrence of that series. The parent's expansion drops the replaced instant;
	// the override renders as its own occurrence through the normal path below.
	...
	for (const occurrence of occurrences) {
		...
		for (const seg of segments) {
			...
			if (bar) {
				...
				// calendar-day math, not a fixed 168h divisor: DST transition weeks are 167/169h
				// long and the fixed divisor mis-buckets every later Sunday one row early (which
				// then clamps into the wrong column)
...
export function getRangeKey(range: EventCalendarDateRange): string {
...
/**
 * Resolves whether a day is an off day (non-working) in the display zone. Callers pass the
 * calendar's own weekendDays so the shading cannot contradict the weekend the rest of the
 * calendar renders; an explicit offDays.weekendDays still wins over it.
 */
export function resolveOffDay(
	day: Date,
	timeZone: string,
...
/** List of zoned day starts covering `range` (end exclusive). */
export function listRangeDays(range: EventCalendarDateRange, timeZone: string): Date[] {
	const result: Date[] = [];
	let cursor = zonedStartOfDay(range.start, timeZone);
	while (cursor < range.end) {
		result.push(cursor);
		cursor = zonedStartOfDay(addDays(toZoned(cursor, timeZone), 1), timeZone);
	}
	return result;
}
