<script lang="ts" module>
	...
	export type ContextUsageInputUsageProps = WithElementRef<HTMLAttributes<HTMLDivElement>>;
</script>
...
<script lang="ts">
	...
	/**
	 * The `Input` row: `usage.inputTokens` in compact figures and, when `cost.inputUsd` is
	...
	let {
		...
		children,
		...restProps
	}: ContextUsageInputUsageProps = $props();

	const state = getContextUsageContext("`<ContextUsage.InputUsage>`");
	const line = $derived(state.line("input"));
</script>

{#if children || line}
	<div
		bind:this={ref}
		data-slot="context-usage-input-usage"
		data-kind="input"
