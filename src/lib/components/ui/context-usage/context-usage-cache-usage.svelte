<script lang="ts" module>
	...
	export type ContextUsageCacheUsageProps = WithElementRef<HTMLAttributes<HTMLDivElement>>;
</script>
...
<script lang="ts">
	...
	/**
	 * The `Cache` row: `usage.cachedInputTokens` in compact figures and, when `cost.cacheUsd` is
	...
	let {
		...
		children,
		...restProps
	}: ContextUsageCacheUsageProps = $props();

	const state = getContextUsageContext("`<ContextUsage.CacheUsage>`");
	const line = $derived(state.line("cache"));
</script>

{#if children || line}
	<div
		bind:this={ref}
		data-slot="context-usage-cache-usage"
		data-kind="cache"
